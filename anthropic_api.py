...
def generate_single(instruction,
    ...
    try:
        ...
        thinking = completion.content[0].thinking
        response = completion.content[1].text
        print(completion.usage.output_tokens)
        full_response = "<think>\n" + thinking + "\n</think>\n\n" + response
    ...
    if estimate:
        ...
        for _ in range(estimate_n_sample):
            test_completion = generate_single(random.choice(
                instructions), model_name, max_tokens, thinking_budget, return_raw_object=True, **anthropic_args)
    ...
    with multiprocessing.Pool() as pool:
        for i in tqdm(range(0, len(instructions), batch_size)):
            ...
            args_iter = zip(batch, repeat(model_name),
                            repeat(max_tokens), repeat(thinking_budget), repeat(False))
